...
if (($_GET['action'] ?? '') === 'migrate') {
    ...
    $stmts = [
        ...
        'plan offer prices'   => "ALTER TABLE subscription_plans ADD COLUMN IF NOT EXISTS plan_type ENUM('INDIVIDUAL','BUNDLE','COURSE_PACK') NOT NULL DEFAULT 'INDIVIDUAL', ADD COLUMN IF NOT EXISTS bundle_description TEXT NULL, ADD COLUMN IF NOT EXISTS price_monthly_offer_cents INT NULL, ADD COLUMN IF NOT EXISTS price_yearly_offer_cents INT NULL, ADD COLUMN IF NOT EXISTS offer_label VARCHAR(80) NULL, ADD COLUMN IF NOT EXISTS offer_ends_at DATETIME NULL",
        'gateway keys on plans' => "ALTER TABLE subscription_plans ADD COLUMN IF NOT EXISTS razorpay_plan_monthly VARCHAR(255) NULL, ADD COLUMN IF NOT EXISTS razorpay_plan_yearly VARCHAR(255) NULL, ADD COLUMN IF NOT EXISTS stripe_price_monthly VARCHAR(255) NULL, ADD COLUMN IF NOT EXISTS stripe_price_yearly VARCHAR(255) NULL",
        'coupon_usages table' => "CREATE TABLE IF NOT EXISTS coupon_usages (id BIGINT AUTO_INCREMENT PRIMARY KEY, coupon_id VARCHAR(64) NOT NULL, user_id VARCHAR(64) NOT NULL, invoice_id VARCHAR(64) NOT NULL DEFAULT '', discount_cents INT NOT NULL DEFAULT 0, used_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE KEY uniq_uc (coupon_id, user_id), INDEX idx_cu_coupon (coupon_id))",
        'payment_orders table'=> "CREATE TABLE IF NOT EXISTS payment_orders (id VARCHAR(64) PRIMARY KEY, user_id VARCHAR(64) NOT NULL, plan_id VARCHAR(64) NOT NULL, billing_cycle ENUM('MONTHLY','YEARLY') NOT NULL, gateway ENUM('RAZORPAY','STRIPE','PAYPAL','PHONEPE','PAYU') NOT NULL, gateway_order_id VARCHAR(255) NOT NULL, amount_cents INT NOT NULL, currency VARCHAR(3) NOT NULL DEFAULT 'INR', coupon_id VARCHAR(64) NULL, discount_cents INT NOT NULL DEFAULT 0, status ENUM('PENDING','PAID','FAILED','EXPIRED') NOT NULL DEFAULT 'PENDING', gateway_payment_id VARCHAR(255) NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, INDEX idx_po_user (user_id), INDEX idx_po_gw (gateway_order_id))",
        // GST + company profile migrations
        'invoice_counter table' => "CREATE TABLE IF NOT EXISTS invoice_counter (year_key VARCHAR(10) PRIMARY KEY, last_seq INT NOT NULL DEFAULT 0)",
        'plan gst columns'      => "ALTER TABLE subscription_plans ADD COLUMN IF NOT EXISTS gst_percent DECIMAL(5,2) NOT NULL DEFAULT 18.00, ADD COLUMN IF NOT EXISTS is_gst_applicable TINYINT(1) NOT NULL DEFAULT 1, ADD COLUMN IF NOT EXISTS is_gst_inclusive TINYINT(1) NOT NULL DEFAULT 1, ADD COLUMN IF NOT EXISTS sac_code VARCHAR(10) NULL DEFAULT '998314'",
        'invoice gst columns'   => "ALTER TABLE invoices ADD COLUMN IF NOT EXISTS subtotal_cents INT NULL, ADD COLUMN IF NOT EXISTS gst_percent DECIMAL(5,2) NULL, ADD COLUMN IF NOT EXISTS cgst_cents INT NULL DEFAULT 0, ADD COLUMN IF NOT EXISTS sgst_cents INT NULL DEFAULT 0, ADD COLUMN IF NOT EXISTS igst_cents INT NULL DEFAULT 0, ADD COLUMN IF NOT EXISTS gst_type ENUM('CGST_SGST','IGST','EXEMPT') NULL, ADD COLUMN IF NOT EXISTS customer_name VARCHAR(255) NULL, ADD COLUMN IF NOT EXISTS customer_email VARCHAR(255) NULL, ADD COLUMN IF NOT EXISTS customer_gstin VARCHAR(15) NULL, ADD COLUMN IF NOT EXISTS customer_address TEXT NULL, ADD COLUMN IF NOT EXISTS place_of_supply VARCHAR(100) NULL, ADD COLUMN IF NOT EXISTS sac_code VARCHAR(10) NULL, ADD COLUMN IF NOT EXISTS notes TEXT NULL",
        'company settings'      => "INSERT IGNORE INTO app_settings (`key`, value, value_type, `group`, label, description, sort_order) VALUES ('company_name','Devithor LMS Pvt Ltd','STRING','company','Company Name','Legal name as on GST certificate',1),('company_address_line1','','STRING','company','Address Line 1','',2),('company_address_line2','','STRING','company','Address Line 2','',3),('company_city','','STRING','company','City','',4),('company_state','','STRING','company','State','e.g. Maharashtra',5),('company_state_code','','STRING','company','State Code','2-digit GST state code e.g. 27',6),('company_pincode','','STRING','company','Pincode','',7),('company_gstin','','STRING','company','GSTIN','15-character GST registration number',8),('company_pan','','STRING','company','PAN Number','',9),('company_cin','','STRING','company','CIN','Company Identification Number (optional)',10),('company_email','billing@apptesting.in','STRING','company','Billing Email','Shown on invoices',11),('company_phone','','STRING','company','Phone','',12),('company_website','https://apptesting.in','STRING','company','Website','',13),('bank_name','','STRING','company','Bank Name','',14),('bank_account_name','','STRING','company','Bank Account Holder Name','',15),('bank_account_number','','STRING','company','Bank Account Number','',16),('bank_ifsc','','STRING','company','IFSC Code','',17),('invoice_prefix','INV','STRING','company','Invoice Prefix','e.g. INV → INV/2026-27/0001',18),('invoice_gst_type','CGST_SGST','STRING','company','GST Type','CGST_SGST (same state) or IGST (inter-state)',19),('invoice_terms','Payment is due upon receipt. This is a computer-generated invoice and does not require a physical signature. GST charged as per applicable Indian tax laws. SAC Code: 998314 (Information Technology Services). Subject to jurisdiction of the company state.','STRING','company','Invoice Terms & Notes','Shown at bottom of every invoice',20)",
    ];
    try {
...
$files = [
    ...
    'src/Controllers/Admin/SubscriptionController.php',
    'src/Controllers/Api/ExamApiController.php',
    'src/Controllers/Api/BillingApiController.php',
    'src/Controllers/Api/LessonInteractionController.php',
    'src/Views/billing/invoice.php',
